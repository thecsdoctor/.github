...
Static content (identity, CV highlights) plus a live `./status --live` section
fetched from real sources (GitHub API, dany.sh visitor counter, Berlin weather,
latest dev.to post). Every fetch has a short timeout and silently degrades —
a failed source just drops its line. CI runs this every 8 h and commits the
result (see .github/workflows/terminal-svg.yml).
"""
import json
import sys
...
def fetch_json(url, timeout=4):
    req = urllib.request.Request(url, headers={"User-Agent": "thecsdoctor-svg/1.0"})
...
def line_github():
    ...
    return ("github", trunc("%d public repos · %s · latest push: %s (%s)"
                            % (len(pubs), "%d star%s" % (stars, "s" if stars != 1 else ""),
                               latest["name"], ago(latest["pushed_at"])), MAXCOLS - 12))
...
def line_berlin():
    ...
    w = d["current_weather"]
    desc = WMO.get(w.get("weathercode"), "unknown sky")
    return ("berlin", "%.0f°C, %s right now" % (w["temperature"], desc))
...
LIVE_FETCHERS = (line_github, line_visitors, line_berlin, line_devto)
...
def build_transcript(live):
    t = [
        ...
        ("blank", ""),
        ("cmd", "head -3 ~/experience.log"),
        ("out", "sr devops eng   accenture, berlin      2024—now    public sector · GenAI on A100 GPUs"),
        ("out", "devops lead     capgemini, berlin      2022—2024   sovereign google cloud, 20+ dev teams"),
        ("out", "software dev    kieback&peter, berlin  2021—2022   YOLOv4 vision testing plugin"),
        ("out", "highlights: cut AWS EC2 costs 28% · ceph SME, 8PB+ storage RFP · air-gapped deploys"),
        ("blank", ""),
        ("cmd", "cat ~/education.log ~/certs.log"),
        ("out", "b.eng. telematics — TH wildau, germany (2018—2022)"),
        ("out", "certs: AWS devops engineer professional · developer associate · cloud practitioner"),
        ("blank", ""),
    ]
    if live:
...
def build_svg(transcript):
    ...
    n_rows = sum(1 for k, _ in transcript if k != "blank") + 1
...
def main():
    ...
    with open("assets/terminal.svg", "w") as f:
        f.write(svg)
    print(f"wrote assets/terminal.svg  {w}x{h}  cycle {total:.1f}s  live lines: {len(lines)}")
